...
def get_raw_text_and_resume_probability(text):
    ...
    try:
        ...
        return raw_text, work_exs, educations, resume_probability
    except:
        return "raw text not present", "no work exs", "no educations", -1
...
def process_batch(batch_df):
    ...
    """
    batch_df[['raw_text', 'work_exs', 'educations', 'resume_probability']] = batch_df['curriculum_json'].apply(
        lambda x: pd.Series(get_raw_text_and_resume_probability(x))
    )
...
def save_to_csv(df, writer):
    ...
    """
    df[['resource_id', 'curriculum_name', 'raw_text']].to_csv(writer, index=False, header=False, escapechar='\\')
...
def main():
    ...
    output_path = "./data/extracted_data/resume_details_curriculum_data.csv"
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as csv_file:
        ...
        writer.writerow(['resource_id', 'curriculum_name', 'raw_text'])

        while True:
            query = f"""
                SELECT resource_id, curriculum_name, curriculum_data 
            ...
            batch_df.rename(columns={"curriculum_data": "raw_text"}, inplace=True)
            save_to_csv(batch_df, csv_file)
